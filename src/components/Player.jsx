...
export const SongControl = ({audio}) => {
    ...
    return (
        <div className="flex gap-x-3 text-xs pt-2">
            <span className="opacity-50 dark:text-white">{formatTime(currentTime)}</span>
            <Slider
            defaultValue={[0]}
            ...
            />
            <span className="opacity-50 dark:text-white">{formatTime(duration)}</span>
        </div>
    );
}
...
const CurrentSong = ({image, title, artists}) => {
    return (
        <div className="flex items-center gap-5 relative overflow-hidden">
            <picture className="w-19 h-16 bg-zinc-800 rounded-md shadow-lg overflow-hidden">
